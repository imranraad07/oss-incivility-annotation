from database import Database

DATABASE_PATH = "annotation.db"
db = Database(DATABASE_PATH)

# db.update_current_issue('ramtin_123', 352750519)
# user = db.get_all_comment_annotations()
# user = db.get_all_issue_annotations()

db.create_user('bobby_456', 0, 352750519, 732267627, 352750519)
db.create_user('bobby_123', 0, 352750519, 869840472, 352750519)
db.create_user('ramtin_123', 0, 352750519, 869840472, 352750519)
db.create_user('imran_123', 0, 352750519, 305814776, 352750519)
db.create_user('admin', 1, '', '', '')
